...
const MenuManagementPage = () => {
  ...
  const handleAddMenu = async (e) => {
    ...
    try {
      ...
      setForm({ name: '', category: '', price: '', status: 'available', image: '', isSpecial: false })
      setImageFile(null)
  ...
  const filteredMenus = menuItems.filter(item =>
    (!filter.category || (filter.category === 'special' ? item.isSpecial : item.category === filter.category)) &&
      ...
      >
        ...
        <div className="mb-3 flex items-center gap-2">
          <input
            ...
            id="isSpecial"
          />
          <label htmlFor="isSpecial" className="font-medium">Là món đặc biệt</label>
        </div>
        <div className="mb-3">
        ...
        <div className="flex justify-end">
          <button
            type="submit"
            className={`bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 ${adding ? 'opacity-60 cursor-not-allowed' : ''}`}
            disabled={adding}
          >{adding ? (isEdit ? 'Đang lưu...' : 'Đang thêm...') : (isEdit ? 'Lưu thay đổi' : 'Thêm món')}</button>
        </div>
      </form>
    </div>
  )
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 p-2">
              {filteredMenus.length === 0 ? (
                ...
                filteredMenus.map((item) => (
                  <div key={item._id} className="bg-white border rounded-lg shadow-sm overflow-hidden flex flex-col justify-between">
                    <img
                      src={item.image}
                      alt={item.name}
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          {item.isSpecial && (
                            <span className="inline-block bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded mt-1 font-semibold">Đặc biệt</span>
                          )}
                        </div>
